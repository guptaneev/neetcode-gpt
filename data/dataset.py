...
class Solution:
    def batch_loader(self, raw_dataset: str, context_length: int, batch_size: int) -> Tuple[List[List[str]], List[List[str]]]:
        ...
        start = torch.randint(len(tokens) - context_length, (batch_size,))

        X, Y = [], []
        for i in range(batch_size):
            X.append(tokens[start[i]:start[i]+context_length])
            Y.append(tokens[start[i]+1:start[i]+1+context_length])
